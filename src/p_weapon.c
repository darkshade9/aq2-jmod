...
//NOTEAM_WOUNDS void setFFState(edict_t *ent);
void weapon_grenade_fire (edict_t *ent, qboolean held);

qboolean CheckPuppetShoot(edict_t *ent)
{
	if  (  (!(ent->client->puppet)) || //resp.toggles & TG_PUPPET)) ||
		( (ent->client->puppet /*resp.toggles & TG_PUPPET*/ ) && (!(ent->client->resp.toggles & TG_PUPPETSHOOT)) )  )
		return true;
	return false;
}
...
void Marker_ProjectSource (gclient_t *client, vec3_t point, vec3_t distance, vec3_t forward, vec3_t right, vec3_t result)
{
        vec3_t  _distance;
...
qboolean Pickup_Weapon (edict_t *ent, edict_t *other)
{
        ...
        else if ( stricmp(ent->item->pickup_name, M3_NAME) == 0 )
        {
                if ( other->client->unique_weapon_total < unique_weapons->value + band ) 
                {
                        other->client->pers.inventory[index]++;                 
                        other->client->unique_weapon_total++;
//PaTMaN-NOBANDAGE                        if ( !(ent->spawnflags & DROPPED_ITEM) )
//PaTMaN-NOBANDAGE                        {
//PaTMaN-NOBANDAGE                                // any weapon that doesn't completely fill up each reload can 
//PaTMaN-NOBANDAGE                                //end up in a state where it has a full weapon and pending reload(s)
//PaTMaN-NOBANDAGE                                if ( other->client->weaponstate == WEAPON_RELOADING ) 
//PaTMaN-NOBANDAGE                                {
//PaTMaN-NOBANDAGE                                        if ( other->client->fast_reload )
//PaTMaN-NOBANDAGE                                        {
//PaTMaN-NOBANDAGE                                                other->client->shot_rds = other->client->shot_max - 2;
//PaTMaN-NOBANDAGE                                        }
//PaTMaN-NOBANDAGE                                        else
//PaTMaN-NOBANDAGE                                               other->client->shot_rds = other->client->shot_max - 1;
//PaTMaN-NOBANDAGE                                }
//PaTMaN-NOBANDAGE                                else
//PaTMaN-NOBANDAGE                                {
//PaTMaN-NOBANDAGE                                        other->client->shot_rds = other->client->shot_max;      
//PaTMaN-NOBANDAGE                                }
//PaTMaN-NOBANDAGE                        }
                        special = 1;
                        gi.cprintf(other, PRINT_HIGH, "%s - Unique Weapon\n", ent->item->pickup_name);
...
/*//PaTMaN - NO AMMO
        ...
        else if ( stricmp(ent->item->pickup_name, MARKER_NAME) == 0 )
        {
                
                if ( other->client->pers.inventory[index] < other->client->marker_max )
                {
                        other->client->pers.inventory[index]++;
                        return true;
                }
...
//PaTMaN-REMOVE_LASTWEAPON2		if (!ent->client->resp.spectating)
//PaTMaN-REMOVE_LASTWEAPON2			ent->client->resp.lastweapon2 = ent->client->newweapon;
...
//FIREBLADE
        ...
        else if(stricmp(ent->client->pers.weapon->pickup_name, MARKER_NAME) == 0)
        {
                ent->client->curr_weap = MARKER_NUM;
        }
        ...
        if ( (ent->client->resp.toggles & TG_SLIPPERS) ) //ent->client->pers.inventory[ITEM_INDEX(item)] )
...
{
        ent->touch = Touch_Item;
        if (deathmatch->value)  //V1.21 REMOVED /*PaTMaN-REMOVE_TEAMPLAY && !teamplay->value*/ && !allweapon->value )
        {
                ent->nextthink = level.time + 74;
                ent->think = ThinkSpecWeap;
        }
//PaTMaN-REMOVE_TEAMPLAY                 else if ( teamplay->value && !allweapon->value)
//PaTMaN-REMOVE_TEAMPLAY                 {
//PaTMaN-REMOVE_TEAMPLAY                         ent->nextthink = level.time + 1000;
//PaTMaN-REMOVE_TEAMPLAY                         ent->think = PlaceHolder;
//PaTMaN-REMOVE_TEAMPLAY                 }
		/*  //V1.21 REMOVED
                else // allweapon set
                {
...
//No MK23 AMMO                ent->client->dual_rds = ent->client->mk23_rds;
        }
        ...
        else if ( stricmp(item->pickup_name, MARKER_NAME) == 0 )
        {
                        ...
                        if ( ((ent->client->pers.weapon == item) ) && (ent->client->pers.inventory[index] == 1) )
                {
                        replacement = FindItem (MK23_NAME); // back to the pistol then
                        ent->client->newweapon = replacement;
                        if ( ent->client->resp.marker_mode ) // hack to avoid an error
                        {
                                ent->client->weaponstate = WEAPON_DROPPING;
                                ent->client->ps.gunframe = 111;
                        }
...
//gi.cprintf(ent,PRINT_HIGH,"WEAPON-GENERIC-START:%i\n",ent->client->ps.gunframe);
        // zucc vwep
...
//FIREBLADE
        if (ent->client->weaponstate == WEAPON_FIRING &&
                ((ent->solid == SOLID_NOT && ent->deadflag != DEAD_DEAD) || 
                 ent->client->resp.toggle_lca /*lights_camera_action*/))
        {
                ...
                case M3_NUM:
                        {
                                ent->client->fired = 0;                                 //reset any firing delays
                                ent->client->burst = 0;
//PaTMaN-NOBANDAGE                                ent->client->fast_reload = 0;
                                break;
                        }
                case HC_NUM:
                        {
...
//PaTMaN-NOBANDAGE                                ent->client->fast_reload = 0;
        ...
        {
			//		gi.cprintf(ent,PRINT_HIGH, "pewp2\n");
			//		gi.cprintf(ent,PRINT_HIGH, "%d\n",ent->deadflag);
            if ( ((ent->client->latched_buttons|ent->client->buttons) & BUTTON_ATTACK) 
				//FIREBLADE
				&& /*(ent->solid != SOLID_NOT || ent->deadflag == DEAD_DEAD) &&*/ !ent->client->resp.toggle_lca /*lights_camera_action*/)
				//FIREBLADE
			{
				...
				case MK23_NUM :
					{
						...
						//PaTMaN - Weapon Ban for Pistol
						
						if ( (int)weaponban->value & WB_PISTOL )
						{
						...
						if ( (ent->client->resp.wmodes & WM_MK23)/*ent->client->resp.mk23_mode != 0*/ && ent->client->fired == 0 )
						{
							ent->client->fired = 1;
							bFire = 1;
						}
						else if (!(ent->client->resp.wmodes & WM_MK23))//ent->client->resp.mk23_mode == 0 )
						{
							bFire = 1;
						}
						break;
				...
				case MP5_NUM:
					{
						...
						//PaTMaN - Weapon Ban for MP5
						
						if ( (int)weaponban->value & WB_MP5 )
						{
							if (ent->client->ps.gunframe == FRAME_IDLE_LAST)
							{
								ent->client->ps.gunframe = FRAME_IDLE_FIRST;
						...
						//No MP5 AMMO                                        if ( ent->client->mp5_rds > 0 )
						//No MP5 AMMO                                        {
						if ( (ent->client->resp.wmodes & WM_MP5)/*mp5_mode != 0*/ && ent->client->fired == 0 && ent->client->burst == 0 )
						{
							ent->client->fired = 1;
						...
						else if ( !(ent->client->resp.wmodes & WM_MP5)/*ent->client->resp.mp5_mode == 0*/ && ent->client->fired == 0 )
						{
							bFire = 1;
						}
						//No MP5 AMMO                                        }
				...
				case M4_NUM:
					{
						...
						//PaTMaN - Weapon Ban for M4
						
						if ( (int)weaponban->value & WB_M4 )
						{
							if (ent->client->ps.gunframe == FRAME_IDLE_LAST)
							{
								ent->client->ps.gunframe = FRAME_IDLE_FIRST;
						...
						if ( (ent->client->resp.wmodes & WM_M4)/*ent->client->resp.m4_mode != 0*/ && ent->client->fired == 0 && ent->client->burst == 0 )
						{
							ent->client->fired = 1;
						...
						else if ( !(ent->client->resp.wmodes & WM_M4)/*ent->client->resp.m4_mode == 0*/ && ent->client->fired == 0 )
						{
							bFire = 1;
						}
						break;
				...
				case HC_NUM:
					{
						...
						//PaTMaN - Weapon Ban for Handcannon
						
						if ( (int)weaponban->value & WB_HC )
						{
							if (ent->client->ps.gunframe == FRAME_IDLE_LAST)
							{
								ent->client->ps.gunframe = FRAME_IDLE_FIRST;
				...
				case SNIPER_NUM:
					{
						...
						//PaTMaN - Weapon Ban for Sniper
						
						if ( (int)weaponban->value & WB_SNIPER )
						{
							if (ent->client->ps.gunframe == FRAME_IDLE_LAST)
							{
								ent->client->ps.gunframe = FRAME_IDLE_FIRST;
				...
				case DUAL_NUM:
					{
						...
						//PaTMaN - Weapon Ban for Akimbo
						
						if ( (int)weaponban->value & WB_AKIMBO )
						{
							if (ent->client->ps.gunframe == FRAME_IDLE_LAST)
							{
								ent->client->ps.gunframe = FRAME_IDLE_FIRST;
						...
						//No DUAL AMMO                                        if ( ent->client->dual_rds > 0 )
						//No DUAL AMMO                                        {
						bFire = 1;
						//No DUAL AMMO                                        }
						//No DUAL AMMO                                        else
...
void weapon_grenade_fire (edict_t *ent, qboolean held)
{
        ...
        if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
//PaTMaN - Dont Subtract Ammo                ent->client->pers.inventory[ent->client->ammo_index]--;
...
void Weapon_Grenade (edict_t *ent)
{
        ...
        if (ent->client->weaponstate == WEAPON_READY)
        {
                ...
                {
                        ent->client->latched_buttons &= ~BUTTON_ATTACK;
//NO AMMO_INDEX                        if (ent->client->pers.inventory[ent->client->ammo_index])
//NO AMMO_INDEX                        {
...
//NO AMMO_INDEX                        else
//NO AMMO_INDEX                        {
//NO AMMO_INDEX                                if (level.time >= ent->pain_debounce_time)
//NO AMMO_INDEX                                {
//NO AMMO_INDEX                                        gi.sound(ent, CHAN_VOICE, gi.soundindex("weapons/noammo.wav"), 1, ATTN_NORM, 0);
//NO AMMO_INDEX                                        ent->pain_debounce_time = level.time + 1;
//NO AMMO_INDEX                                }
//NO AMMO_INDEX                                NoAmmoWeaponChange (ent);
//NO AMMO_INDEX                        }
                        return;
                }
...
//NO AMMO_INDEX        if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
//NO AMMO_INDEX                ent->client->pers.inventory[ent->client->ammo_index]--;
}
...
//NO AMMO_INDEX                }
                ...
                ent->client->ps.gunframe++;
//NO AMMO_INDEX                if (ent->client->ps.gunframe == 12 && ent->client->pers.inventory[ent->client->ammo_index])
//NO AMMO_INDEX                        ent->client->ps.gunframe = 6;
        }
...
/*//NO AMMO_INDEX        if (ent->client->pers.inventory[ent->client->ammo_index] < 1)
        {
...
//NO AMMO_INDEX        else if ((ent->client->ps.gunframe == 21) && (ent->client->buttons & BUTTON_ATTACK)
//NO AMMO_INDEX                && ent->client->pers.inventory[ent->client->ammo_index])
//NO AMMO_INDEX        {
//NO AMMO_INDEX                ent->client->ps.gunframe = 15;
//NO AMMO_INDEX        }
        else
        {
                ent->client->ps.gunframe++;
...
//NO AMMO_INDEX        if (ent->client->pers.inventory[ent->client->ammo_index] < shots)
//NO AMMO_INDEX                shots = ent->client->pers.inventory[ent->client->ammo_index];
...
//NO AMMO_INDEX        if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
//NO AMMO_INDEX                ent->client->pers.inventory[ent->client->ammo_index] -= shots;
}
...
//NO AMMO_INDEX        if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
//NO AMMO_INDEX                ent->client->pers.inventory[ent->client->ammo_index] -= 2;
}
...
//NO AMMO_INDEX        if (ent->client->pers.inventory[ent->client->ammo_index] < 50)
//NO AMMO_INDEX        {
//NO AMMO_INDEX                ent->client->ps.gunframe++;
//NO AMMO_INDEX                return;
//NO AMMO_INDEX        }
 ...
 //NO AMMO_INDEX       if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
//NO AMMO_INDEX                ent->client->pers.inventory[ent->client->ammo_index] -= 50;
}
...
int AdjustSpread( edict_t * ent, int spread )
{
        ...
        if ((ent->client->resp.toggles & TG_LASER) //_laser == 1 //PaTMaN - Check if it is ON, not if the person has it
...
//                ent->client->dual_rds--;
		...
		*///No MK23 AMMO
		...
		//if (!(ent->client->resp.toggles & TG_PUPPET))//_puppet != 1)
//		if ((!(ent->client->resp.toggles & TG_PUPPET)) || ((ent->client->resp.toggles & TG_PUPPET) && (!(ent->client->resp.toggles & TG_PUPPETSHOOT))))
		if (CheckPuppetShoot(ent))
			fire_bullet (ent, start, forward, damage, kick, spread, spread,MOD_MK23);
//                ent->client->mk23_rds--;
//                ent->client->dual_rds--;
//No MK23 AMMO        }
...
void MP5_Fire(edict_t *ent)
{
        ...
        if ( ent->client->burst == 0 && !(ent->client->resp.wmodes & WM_MP5) )//ent->client->resp.mp5_mode) )
        {
                if (ent->client->ps.gunframe == 12)
        ...
        //burst mode
        else if ( ent->client->burst == 0 && (ent->client->resp.wmodes & WM_MP5) )//ent->client->resp.mp5_mode )
        {
...
void M4_Fire(edict_t *ent)
{
        ...
        if ( ent->client->burst == 0 && (ent->client->burst != 2) && !(ent->client->resp.wmodes & WM_M4))//ent->client->resp.m4_mode) )
        {
                if (ent->client->ps.gunframe == 12)
                        ent->client->ps.gunframe = 11;
                else
        ...
        //burst mode
        else if ( ent->client->burst == 0 && (ent->client->resp.wmodes & WM_M4) )//ent->client->resp.m4_mode )
        {
...
//NOTEAM_WOUNDS	setFFState(ent);

        InitShotgunDamageReport();  //FB 6/3/99

                fire_shotgun (ent, start, forward, damage, kick, 800, 800, 12, MOD_M3);
...
//NO AMMO_INDEX//      if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
//NO AMMO_INDEX//              ent->client->pers.inventory[ent->client->ammo_index] -= 2; // PaTMaN
...
//FIREBLADE
      /*//PaTMaN-NORELOADING  
        ...
        *///PaTMaN-NORELOADING
        if (ent->client->weaponstate == WEAPON_DROPPING)
        {
                if (ent->client->resp.wmodes & WM_KNIFE)//knife_mode == 1)
                {
        ...
        if (ent->client->weaponstate == WEAPON_ACTIVATING)
        {
                ...
                if ( (/*(ent->client->resp.knife_mode == 1)*/ (ent->client->resp.wmodes & WM_KNIFE)) && ent->client->ps.gunframe == 0 )
                {
//                      gi.cprintf(ent, PRINT_HIGH, "NewKnifeFirst\n");
                        ent->client->ps.gunframe = FRAME_PREPARETHROW_FIRST;
                        return;
                }
 /*               if ( ent->client->resp.marker_mode == 0 && ent->client->ps.gunframe == 0 )
                {
//                      gi.cprintf(ent, PRINT_HIGH, "NewKnifeFirst\n");
                        ent->client->ps.gunframe = FRAME_PREPARETHROW_FIRST;
                        return;
...
//              gi.cprintf(ent, PRINT_HIGH, "After increment frames = %d\n", ent->client->ps.gunframe);
        ...
        // bandaging case
//PaTMaN-NOBANDAGE        if ( (ent->client->bandaging)
//PaTMaN-NOBANDAGING                if ( (ent->client->weaponstate != WEAPON_FIRING) 
//PaTMaN-NOBANDAGING                && (ent->client->weaponstate != WEAPON_BURSTING)
//PaTMaN-NOBANDAGING                && (ent->client->weaponstate != WEAPON_BUSY) )
//PaTMaN-NOBANDAGING                && (ent->client->weaponstate != WEAPON_BANDAGING) )
//PaTMaN-NOBANDAGING        {
//PaTMaN-NOBANDAGING                ent->client->weaponstate = WEAPON_BANDAGING;
//PaTMaN-NOBANDAGING                ent->client->ps.gunframe = FRAME_DEACTIVATE_FIRST;
//PaTMaN-NOBANDAGING                return;
//PaTMaN-NOBANDAGING        }
        ...
        if ( ent->client->weaponstate == WEAPON_BUSY )
        {
			...
			//PaTMaN-NOBANDAGE             if ( ent->client->bandaging == 1 )
			//PaTMaN-NOBANDAGE                {
			//PaTMaN-NOBANDAGE                        if ( !(ent->client->idle_weapon) )
			//PaTMaN-NOBANDAGE                        {
			//PaTMaN-NOBANDAGE                                Bandage( ent );
			//PaTMaN-NOBANDAGE                                //ent->client->weaponstate = WEAPON_ACTIVATING;
			//PaTMaN-NOBANDAGE                //                ent->client->ps.gunframe = 0;
			//PaTMaN-NOBANDAGE                        }
			//PaTMaN-NOBANDAGE                        else
			//PaTMaN-NOBANDAGE                                (ent->client->idle_weapon)--;
			//PaTMaN-NOBANDAGE                        return;
			//PaTMaN-NOBANDAGE                }
			...
			if ( !(ent->client->idle_weapon) )//&& ent->client->bandage_stopped )
			{
				ent->client->weaponstate = WEAPON_ACTIVATING;
				ent->client->ps.gunframe = 0;
				//PaTMaN-NOBANDAGE                                        ent->client->bandage_stopped = 0;
				return;
			}
			//PaTMaN-NOBANDAGE                                else if ( ent->client->bandage_stopped == 1 )
			//PaTMaN-NOBANDAGE                                {
			//PaTMaN-NOBANDAGE                                        (ent->client->idle_weapon)--;
			//PaTMaN-NOBANDAGE                                        return;
			//PaTMaN-NOBANDAGE                                }
		...
		//PaTMaN-NOBANDAGE        if ( ent->client->weaponstate == WEAPON_BANDAGING )
		//PaTMaN-NOBANDAGE        {
		//PaTMaN-NOBANDAGE                if (ent->client->ps.gunframe == FRAME_DEACTIVATE_LAST )
		//PaTMaN-NOBANDAGE                {
		//PaTMaN-NOBANDAGE                        ent->client->weaponstate = WEAPON_BUSY;
		//PaTMaN-NOBANDAGE                        ent->client->idle_weapon = BANDAGE_TIME;
		//PaTMaN-NOBANDAGE                        return;
		//PaTMaN-NOBANDAGE                }
		//PaTMaN-NOBANDAGE                ent->client->ps.gunframe++;
		//PaTMaN-NOBANDAGE                return;
		//PaTMaN-NOBANDAGE        }
			...
			&& (ent->client->weaponstate != WEAPON_BUSY ) )
        {
			...
			if ( ent->client->resp.wmodes & WM_KNIFE)//knife_mode == 1)// || ent->client->resp.marker_mode == 0) // throwing mode
			{
        ...
        if (ent->client->weaponstate == WEAPON_READY)
        {
				...
				//FIREBLADE
			{
				...
				//PaTMaN - Weapon Ban for Knives
				
				if ( (int)weaponban->value & WB_KNIVES )
				{
					if (ent->client->ps.gunframe == FRAME_IDLE_LAST)
					{
                        ent->client->ps.gunframe = FRAME_IDLE_FIRST;
				...
				if ( ent->client->resp.wmodes & WM_KNIFE)//knife_mode == 1 )// || ent->client->resp.knife_mode == 0)
				{
...
//        gitem_t*        item;
        ...
        if ( !(ent->client->resp.wmodes & WM_KNIFE))//ent->client->resp.knife_mode == 0 )
        {
                knife_return = knife_attack (ent, start, forward, damage, kick  );

                if ((knife_return < ent->client->knife_sound) || (knife_return == 0))
...
ent->client->ps.gunframe = 111;
ent->client->weaponstate = WEAPON_BUSY;
//This will never happen                item = FindItem(KNIFE_NAME);
//PaTMaN - Dont Subtract Ammo                ent->client->pers.inventory[ITEM_INDEX(item)]--;
...
//                        return 0;
//                }
//PaTMaN-NORELOADING                else
//PaTMaN-NORELOADING                {
//PaTMaN-NORELOADING                        ent->client->weaponstate = WEAPON_RELOADING;
//PaTMaN-NORELOADING                        ent->client->ps.gunframe = 111;
//PaTMaN-NORELOADING                }
...
void Weapon_Generic_Marker(edict_t *ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST, 
                                         int FRAME_IDLE_LAST, int FRAME_DEACTIVATE_LAST, 
                                         int FRAME_PREPARETHROW_LAST, int FRAME_IDLE2_LAST,
                                         int FRAME_THROW_LAST, int FRAME_STOPTHROW_LAST,
                                         int FRAME_NEWKNIFE_LAST,
                                         int *pause_frames, int *fire_frames, int (*fire)(edict_t *ent))
{
        ...
        if (ent->client->weaponstate == WEAPON_ACTIVATING)
        {
                ...
                if ( (!(ent->client->resp.wmodes & WM_MARKER))/*marker_mode == 0)*/ /* || (ent->client->resp.marker_mode == 0)*/ && ent->client->ps.gunframe == 0 )
                {
                        ent->client->ps.gunframe = FRAME_PREPARETHROW_FIRST;
                        return;
                }
...
//PaTMaN-NOBANDAGE                                        ent->client->bandage_stopped = 0;
                ...
                && (ent->client->weaponstate != WEAPON_BUSY ) )
        {
                ...
                if ( !(ent->client->resp.wmodes & WM_MARKER))//marker_mode == 0)// || ent->client->resp.marker_mode == 0) // throwing mode
                {
                        ent->client->ps.gunframe = FRAME_NEWKNIFE_LAST;
                        // zucc more vwep stuff
                        if((FRAME_NEWKNIFE_LAST - FRAME_NEWKNIFE_FIRST) < 4)
        ...
        if (ent->client->weaponstate == WEAPON_READY)
        {
			...
			if (ent->client->resp.wmodes & WM_MARKER) //delete mode
				T_HighlightJumpMarkerRadius(ent,100,false);
...
//PaTMaN - Weapon Ban for Knives
                ...
                ent->client->ps.gunframe++;
                        ...
                        if ( !(ent->client->resp.wmodes & WM_MARKER))//ent->client->resp.marker_mode == 0 )// || ent->client->resp.knife_mode == 0)
                        {
                                ent->client->ps.gunframe = FRAME_THROW_FIRST;
                        }
                        else
...
int Marker_Fire (edict_t *ent)
{
        vec3_t          start, v;
        vec3_t          forward, right;
        vec3_t          offset;
        int             damage = 0;//200;
//        int             throwndamage = 0;//250;
        int             kick = 0;//50; // doesn't throw them back much..
//        int             marker_return = 3;
        gitem_t*        item;

if ((int)weaponban->value & WB_MARKER) return 1;
...
/*        if ( ent->client->resp.marker_mode == 0 )
        {
                marker_return = knife_attack (ent, start, forward, damage, kick  );
                
                if (marker_return < ent->client->knife_sound)
                        ent->client->knife_sound = marker_return;
    ...
    */    {
                ...
                gi.sound(ent, CHAN_WEAPON, gi.soundindex("weapons/throw.wav"), 1, ATTN_NORM, 0);
                          ...
                          //      VectorSet(offset, 24, 8, ent->viewheight);
                                VectorSet(offset, 14, 0, ent->viewheight);
                                VectorAdd (offset, vec3_origin, offset);
                          //      forward[2] += .17;
                                ...
                                Marker_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
          ...
          /*      if ( ent->client->pers.inventory[ITEM_INDEX(item)] <= 0 )
                {
                        ...
                        ChangeWeapon( ent );
                                                // zucc was at 1250, dropping speed to 1200
                        marker_throw (ent, start, forward, damage, 1200 ); 
                        return 0;
                }
                else
                {
...
//                vec3_t angles;
        ...
        VectorSet(offset,24 , 8, ent->viewheight- height);
		if (ent->client->resp.toggle_laser == 1) // PaTMaN 
{
        ...
        if (tr.fraction < 1) {
                VectorMA(tr.endpos,-70,forward,endp);
...
/*
            ...
            */     //       ent->client->weaponstate = WEAPON_RELOADING;
				if (ent->client->resp.wmodes & WM_MARKER)//marker_mode == 1)
				{
				    ent->client->weaponstate = WEAPON_READY;
				}
//PaTMaN-NORELOADING				else
...
//PaTMaN-NORELOADING				}
            ...
            /*    AngleVectors (ent->client->v_angle, forward, right, NULL);
              ...
              */  
                //      fire_rocket (ent, start, forward, damage, 650, 200, 200);
                // if (ent->client->resp.marker_mode == 1)

					marker_throw (ent, start, forward, damage, 1200 , false, ent->client->resp.marker_color, true);
                // else
			 	//	marker_throw (ent, start, forward, damage, 1200 ); 
                //here
                // 
...
void Weapon_Marker (edict_t *ent)
{
        ...
        static int      fire_frames[]   = {6, 7, 8, 9, 10, 105, 0};
        // I think we need a special version of the generic function for this...
        Weapon_Generic_Marker (ent, 5, 12, 52, 59, 63, 102, 105, 110, 117, pause_frames, fire_frames, Marker_Fire);
}
...
if ((int)weaponban->value & WB_GRENADE) return;
...
//PaTMaN-NORELOADING        {
//PaTMaN-NORELOADING                ent->client->weaponstate = WEAPON_RELOADING;
//PaTMaN-NORELOADING                ent->client->ps.gunframe = 0;
//PaTMaN-NORELOADING        }
...
//PaTMaN-NOBANDAGE                else if ( ent->client->bandage_stopped )
//PaTMaN-NOBANDAGE                        (ent->client->idle_weapon)--;
                
                
       }
        
//PaTMaN-NOBANDAGING        if ( ent->client->weaponstate == WEAPON_BANDAGING )
//PaTMaN-NOBANDAGING        {
//PaTMaN-NOBANDAGING                if (ent->client->ps.gunframe == 0 )
//PaTMaN-NOBANDAGING                {
//PaTMaN-NOBANDAGING                        ent->client->weaponstate = WEAPON_BUSY;
//PaTMaN-NOBANDAGE                        ent->client->idle_weapon = BANDAGE_TIME;
//PaTMaN-NOBANDAGING                        return;
//PaTMaN-NOBANDAGING                }
//PaTMaN-NOBANDAGING                ent->client->ps.gunframe--;
//PaTMaN-NOBANDAGING                return;
//PaTMaN-NOBANDAGING        }

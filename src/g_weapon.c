        ...
        if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))
        {
                if (tr.fraction < 1.0)
                {
                        if (tr.ent->takedamage)
                        {
							if (tr.ent->client)
							{
								if (tr.ent->client->resp.toggles & TG_SHOOTABLE)
                ...
                if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))
                {
                        if (tr.fraction < 1.0)
                        {
                                ...
                                if ((tr.ent->svflags & SVF_MONSTER)
									|| (tr.ent->client)
									|| (!Q_stricmp(tr.ent->classname,"puppet"))
									|| (!Q_stricmp(tr.ent->classname,"pup")))
                                {
                                        ignore = tr.ent;
...
void fire_grenade2 (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held)
{
        ...
        {
	//			gi.centerprintf(grenade->owner,"1: %f\n",timer);
	//			gi.cprintf(grenade->owner,PRINT_HIGH,"1: %f\n", timer);
                gi.sound (self, CHAN_WEAPON, gi.soundindex ("weapons/hgrent1a.wav"), 1, ATTN_NORM, 0);
                gi.linkentity (grenade);
        }

	//	gi.centerprintf(grenade->owner,"1: %d\n", timer);
}
...
void knife_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
		...
		if (other->client)
			if (!(other->client->resp.toggles & TG_SHOOTABLE))
				return;

//        if ((!Q_stricmp(other->classname, "puppet")) || //V1.21 FIX - 1.20.006.01
//			(!Q_stricmp(other->classname, "pup"))) //V1.21 FIX - 1.20.006.01
//			return; //V1.21 FIX - 1.20.006.01
...
//glass fx
        ...
        if (0 == Q_stricmp(other->classname, "func_explosive")) /* || //V1.21 FIX - 1.20.006.01
			(!Q_stricmp(other->classname, "puppet")) || //V1.21 FIX - 1.20.006.01
			(!Q_stricmp(other->classname, "pup")))*/ //V1.21 FIX - 1.20.006.01
          {
...
// ---
		...
		//	if (other->client)
		//	{
		//		if (other->client->resp.toggles & TG_SHOOTABLE)//TG_TEAMJUMP))//_solid == 0)

...
void    ServerCommand (void)
{
        ...
        if (Q_stricmp (cmd, "addip") == 0)
                ...
                SVCmd_ReloadMOTD_f();
		else if (!Q_stricmp (cmd, "killvote"))
		{
			if (level.voting < 1)
		...
		else if (!Q_stricmp (cmd, "addmap"))
		{
			if (gi.argc() > 2)
			{
				...
				gi.cprintf (NULL, PRINT_HIGH, "Wrong syntax: addmap <mapname>\n");
		}
		else if (!Q_stricmp (cmd, "removemap"))
		{
			if (gi.argc() > 2)
			{
				strtok(gi.args()," ");
				RemoveMap(strtok(NULL," "));
			}
			else
				gi.cprintf (NULL, PRINT_HIGH, "Wrong syntax: removemap <mapname>\n");
		}
		else if (!Q_stricmp (cmd, "fdelete"))
			FileDelete(NULL);
		else if (!Q_stricmp (cmd, "frename"))
			FileRename(NULL);
		else

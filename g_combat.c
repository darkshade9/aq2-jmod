...
//NOTEAM_WOUNDS void Add_TeamWound( edict_t *attacker, edict_t *victim, int mod);
...
//PaTMaN-REMOVE-BLEED        if ( targ->client )
//PaTMaN-REMOVE-BLEED        {
//PaTMaN-REMOVE-BLEED                targ->client->bleeding = 0;
//PaTMaN-REMOVE-BLEED                //targ->client->bleedcount = 0;
//PaTMaN-REMOVE-BLEED                targ->client->bleed_remain = 0;
//PaTMaN-REMOVE-BLEED        }
...
//player,NULL,NULL,dir(-viewangles),point?,normal?,damage(200),200,DAMAGE_BULLET,mod(MOD_SNIPER)
...
void T_Damage_Puppet (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
//        gclient_t       *client;
        int                     take;
        int                     save;
...
//        int                     psave;
        ...
        if ((!Q_stricmp(targ->classname,"puppet")) ||
			(!Q_stricmp(targ->classname,"pup")))
        { 
                if (!((targ != attacker) && 
                      ...
                      ((int)(dmflags->value) & (DF_NO_FRIENDLY_FIRE)))))
                {
					...
					{ 
						...
						//	gi.cprintf(targ, PRINT_HIGH, "Head damage\n"); 
							if (attacker->client)
								gi.cprintf(attacker, PRINT_HIGH, "You hit %s's %s in the head\n", targ->lastowner->client->pers.netname,targ->classname);
						...
						{ 
							...
							if (attacker->client)
								gi.cprintf(attacker, PRINT_HIGH, "You hit %s's %s in the legs\n", targ->lastowner->client->pers.netname,targ->classname);
						...
						{ 
							...
							if (attacker->client)
								gi.cprintf(attacker, PRINT_HIGH, "You hit %s's %s in the stomach\n", targ->lastowner->client->pers.netname,targ->classname);
						...
						//				targ->classname); //client->pers.netname);
						...
						//	{
								...
								if (attacker->client)
									gi.cprintf(attacker, PRINT_HIGH, "You hit %s's %s in the chest\n", targ->lastowner->client->pers.netname,targ->classname);
								damage_type = LOC_CDAM;
						//	}
...
void T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
        ...
        int                                             do_sparks = 0;
        int                     damage_type = 0; // used for MOD later
//PaTMaN-REMOVE-BLEED        int                     bleeding = 0; // damage causes bleeding
        int                     head_success = 0;
        int                                             instant_dam = 1;
		...
		if ((!Q_stricmp(targ->classname,"puppet")) ||
			(!Q_stricmp(targ->classname,"pup")))
		{
			T_Damage_Puppet (targ,inflictor,attacker,dir,point,normal,damage,knockback,dflags,mod);
			return;
		}
        ...
        {
			if (targ->client)
				if (targ->client->resp.toggle_lca)
					return;
        }
        //FIREBLADE
        ...
        { 
                      ...
                      ((int)(dmflags->value) & (DF_NO_FRIENDLY_FIRE)))))
                {
                        ...
                        { 
                                ...
                                if (from_top < 0.0)      //FB 6/1/99
                                        from_top = 0.0;  //Slightly negative values were being handled wrong
//PaTMaN-REMOVE-BLEED                                bleeding = 1;   
                                instant_dam = 0;
                                ...
                                { 
                                        ...
                                        damage_type = LOC_LDAM;
//PaTMaN-NOLEGDAMAGE                                        targ->client->leg_damage = 0; //PaTMaN - No Leg Damage
//PaTMaN-NOLEGDAMAGE                                        targ->client->leghits++;
                                } 
                                else if (z_rel < STOMACH_DAMAGE)
                                { 
                                        damage = damage * .4; 
                                ...
                                {   
                                        if ( (targ->client->resp.toggles & TG_VEST)
												//(targ->client->pers.inventory[ITEM_INDEX(item)])
                                                && mod != MOD_KNIFE
                                                && mod != MOD_KNIFE_THROWN
                                                && mod != MOD_SNIPER )
                                        {
                                                if (attacker->client)
                                                {
													...
													gi.cprintf(targ, PRINT_HIGH, "Kevlar Vest absorbed most of %s's puppet's shot\n", 
                                                                attacker->lastowner->client->pers.netname);

                                                gi.sound(targ, CHAN_ITEM, gi.soundindex("misc/vest.wav"), 1, ATTN_NORM, 0);
                                                damage = (int)(damage/10);
                                                damage_type = LOC_CDAM;
//PaTMaN-REMOVE-BLEED                                                bleeding = 0;
                                                instant_dam = 1;
                                                stopAP = 1;                                                                                             do_sparks = 1;
                                        }
                                        else if ( (targ->client->resp.toggles & TG_VEST)
											//(targ->client->pers.inventory[ITEM_INDEX(item)])
                                                && mod == MOD_SNIPER )
                                        {
                                                if ( attacker->client )
                                                {
													...
													gi.cprintf(targ, PRINT_HIGH, "Kevlar Vest absorbed some of %s's puppet's AP sniper round\n",
                                                                attacker->lastowner->client->pers.netname);
...
//NOTEAM_WOUNDS				if (team_round_going && attacker->client && targ != attacker && OnSameTeam(targ, attacker))
//NOTEAM_WOUNDS				{
//NOTEAM_WOUNDS					Add_TeamWound(attacker, targ, mod);
//NOTEAM_WOUNDS				}
            }   
        }
...
//                        {
                                gi.cprintf(targ, PRINT_HIGH, "Leg damage\n"); 
//PaTMaN-NOLEGDAMAGE                                targ->client->leg_damage = 1;
//PaTMaN-NOLEGDAMAGE								targ->client->leghits++;
//PaTMaN-REMOVE-BLEED                //      bleeding = 1; for testing
//                        }
        }
		...
		// check for godmode
		if ( /*(targ->flags & FL_GODMODE) && */!(dflags & DAMAGE_NO_PROTECTION) && (targ->client) )
        {
                        take = 0;
                        save = damage;
						if (instant_dam)
...
//PaTMaN-REMOVE-BLEED                        bleeding = 1;
        ...
        if (client)
        {
                        ...
                        //zucc handle adding bleeding here
//PaTMaN-REMOVE-BLEED                        if ( damage_type && bleeding ) // one of the hit location weapons
//PaTMaN-REMOVE-BLEED                        {
...
//PaTMaN-REMOVE-BLEED                                client->bleeding += damage*BLEED_TIME;
//PaTMaN-REMOVE-BLEED                                VectorSubtract (point, targ->absmax, targ->client->bleedloc_offset);
//PaTMaN-REMOVE-BLEED                                //VectorSubtract(point, targ->s.origin,  client->bleedloc_offset);
//PaTMaN-REMOVE-BLEED                                
//PaTMaN-REMOVE-BLEED                        }
//PaTMaN-REMOVE-BLEED                        else if ( bleeding )
//PaTMaN-REMOVE-BLEED                        {
//PaTMaN-REMOVE-BLEED
//Bandage(targ); //PaTMaN
//return;
//PaTMaN-REMOVE-BLEED
//PaTMaN-REMOVE-BLEED                        /*
//PaTMaN-REMOVE-BLEED                        if ( client->bleeding < damage*BLEED_TIME )
//PaTMaN-REMOVE-BLEED                        {
//PaTMaN-REMOVE-BLEED                        client->bleeding = damage*BLEED_TIME;
//PaTMaN-REMOVE-BLEED                        //client->bleedcount = 0;
//PaTMaN-REMOVE-BLEED                        }*/
//PaTMaN-REMOVE-BLEED                                client->bleeding += damage*BLEED_TIME;
//PaTMaN-REMOVE-BLEED                                VectorSubtract (point, targ->absmax, targ->client->bleedloc_offset);
//PaTMaN-REMOVE-BLEED                                //VectorSubtract(point, targ->s.origin,  client->bleedloc_offset);
...
//PaTMaN-REMOVE-BLEED                        }
                        if ( attacker->client )
                        {
//PaTMaN-REMOVE                                attacker->client->resp.damage_dealt += damage; 
                      ...
                      CGF_SFX_ShootBreakableGlass(ent, inflictor, 0, mod);
                        ...
                        if (CanDamage (ent, inflictor))
                        {
							...
							if (ent->client)
							{
								char buf[8]="";
								//inflictor = grenade
								//ent = person getting hit
								//inflictor->owner = thrower

								if (inflictor->owner)
								{
									sprintf(buf," %i ",inflictor->owner->client->resp.id);
									if (!strstr(ent->client->resp.grenadejumplist,buf))
									{	//id not found in list
										if (inflictor->owner == ent)	//if own grenade
										{
											if (ent->client->resp.toggles & TG_GRENJUMP) //if gren jumping for own
												T_Damage (ent, inflictor, attacker, dir, ent->s.origin, vec3_origin, (int)(points*.75), (int)(points*.75), DAMAGE_RADIUS, mod); //do tha damage
											else //else if no gren jumping for own
												T_Damage (ent, inflictor, attacker, dir, ent->s.origin, vec3_origin, 0, 0, DAMAGE_RADIUS, mod); //don't damage
										}
										else //if somebody else's grenades
										{
											if (ent->client->resp.toggles & TG_GRENJUMPOTHER) //if gren jumping for other
												T_Damage (ent, inflictor, attacker, dir, ent->s.origin, vec3_origin, (int)(points*.75), (int)(points*.75), DAMAGE_RADIUS, mod); //do tha damage
											else //else if no gren jumping for other
												T_Damage (ent, inflictor, attacker, dir, ent->s.origin, vec3_origin, 0, 0, DAMAGE_RADIUS, mod); //don't damage
										}
									}
									else	//id found in list so go boom!
										T_Damage (ent, inflictor, attacker, dir, ent->s.origin, vec3_origin, (int)(points*.75), (int)(points*.75), DAMAGE_RADIUS, mod); //do tha damage
								}
								else	//explosions from world
									T_Damage (ent, inflictor, attacker, dir, ent->s.origin, vec3_origin, (int)(points*.75), (int)(points*.75), DAMAGE_RADIUS, mod); //do tha damage
							}
						}
				}
